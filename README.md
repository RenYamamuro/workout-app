...
4. **推定1RMと自己ベスト** — 入力中のセットから Epley式（`1RM = 重量 ×(1 + レップ/30)`）で
...
5. **データの書き出し／読み込み** — JSONファイルでバックアップ。端末やブラウザを移るときにも使う
6. **種目ごとの図解** — 19種目それぞれにマシンの図（SVG）を表示。タップで拡大
7. **種目ごとの履歴** — 図解をタップすると、その種目だけの記録が新しい順に並ぶ。
   推定1RMと自己ベスト（★）付き。履歴画面の種目名からも開ける。
   図解は小さく表示され、タップすると拡大する（履歴を主役にするため）
8. **実物の写真を登録** — 拡大表示から自分で撮った写真を登録すると、図の代わりに表示される
